...
Flask is the API. Next.js is the UI.
...
Flask listens on `http://127.0.0.1:5000` (JSON only). Next.js proxies `/api/*` to Flask.
...
```
traffic-app/
├── app.py                 # Flask REST API (predictions, PPO, YOLO, map JSON)
├── utils/                 # Python helpers (including Leaflet map payload)
├── data/                  # OSM edge CSV (tvm_edges.csv)
├── models/                # Optional YOLO / hybrid weights
├── scripts/               # Network fetch and YOLO CLI helpers
├── static/uploads/        # YOLO annotated images (served at /static/uploads)
└── web/                   # Next.js 16 + TypeScript + Tailwind UI
```
